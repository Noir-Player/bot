...
class LavalinkWebsocket:
    ...
    async def _update_handler(self, raw: dict) -> None:
        await self._bot.wait_until_ready()

        try:
            event = DiscordEvent.model_validate(raw)

            data = event.d

            if isinstance(data, VoiceServerUpdate):
                guild_id = int(data.guild_id)
                player = self.get_player(guild_id)

                if player:
                    await player.on_voice_server_update(data)

            elif isinstance(data, VoiceStateUpdate):
                if int(data.user_id) != self.user_id or not data.guild_id:
                    return
            ...
            self._log.warning(f"Failed to handle event: {raw}")
    ...
    async def _handle_ws_msg(self, data: dict) -> None:
        ...
        if op == "ready":
            ...
            event = getattr(wsmodels, data.get("type")).model_validate(data)  # type: ignore
        ...
        while True:
            try:
                ...
                if not self.is_connected:
                    self._loop.create_task(self._node.connect(reconnect=True))
