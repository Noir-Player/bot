...
class YoutubeMusicSearch(BaseSearch):
    ...
    async def album(self, id: str, **kwargs) -> Album | None:
        ...
        if not isinstance(from_lavalink, Playlist):
            return
    ...
    async def relayted(
        ...
        if isinstance(song_or_playlist_id, Track):
            raw = self.client.get_watch_playlist(
                song_or_playlist_id.info.identifier, radio=True, limit=2
            )
        else:
            raw = self.client.get_watch_playlist(
        ...
        relayted = self.client.get_song_related(raw["related"])  # type: ignore
    ...
    async def lyrics(self, song: Track, **kwargs) -> Track | None:
        ...
        lyrics = self.client.get_lyrics(raw["lyrics"]).get("lyrics")  # type: ignore
