...
from services.app.router import Router
from services.broker import Broker
from services.database.core import Database
from services.database.models import AddTrackRequest

players = Router(
    "players", __name__, static_folder="static", template_folder="templates"
)
...
class Executor:
    ...
    def __init__(self) -> None:
        ...
        self.queue_keys = ["clear", "jump", "remove", "move"]
    ...
    async def start(self):
        self.player: NoirPlayer = players.bot.node.get_player(
            await fetch_player(int(session["me"].get("id")))
        )

        self.user = players.execute(
            players.bot.fetch_user(int(session["me"].get("id")))
        )
    ...
    def play_track(self, data):
        query = players.execute(
            players.bot.node.get_tracks(data.get("query"), requester=self.user)
        )
    ...
    def play_playlist(self, data):
        try:
            ...
            for track in playlist.get("tracks"):
                try:
                    query = players.execute(
                        players.bot.node.get_tracks(
                            track.get("url"), requester=self.user
                        )
                    )[0]
    ...
    def play_recommendations(self, data):
        if self.player.current and self.player.current.track_type.value in [
            "spotify",
            "youtube",
        ]:
            query = players.execute(
                players.bot.node.get_recommendations(
            ...
            players.execute(
                self.player.queue.put_list(
                    query.tracks if type(query) == pomice.Playlist else query
                )
            )
            ...
            try:
                ...
                if action != "destroy" and (not self.player or not self.connect):
                    ...
                    if voice and not players.bot.node.get_player(voice.guild.id):
                        ...
                        players.execute(
                            voice.guild.change_voice_state(
                                channel=voice, self_deaf=True
                            )
                        )
                        ...
                        await websocket.send(
                            json.dumps({"message": "Voice not found", "code": 404})
                        )
                        continue
                ...
                if action == "destroy":
                    ...
                    if self.player.current:
                        db.stars.add_to_stars(
                            int(session["me"].get("id")), self.player.current_build
                        )
                    ...
                    players.execute(
                        self.player.queue.remove(
                            data.get("pos")
                            if data.get("pos") != -1
                            else self.player.current
                        )
                    )
                    ...
                    try:
                        players.execute(
                            self.player.play(self.player.queue.jump(data.get("pos")))
                        )
                    except:
                        pass
    ...
    async def _consume(self) -> None:
        ...
        broker = Broker(
            Redis(host=HOST, port=PORT, password=PASS), self.player.guild.id
        )
...
async def current():
    ...
    player: NoirPlayer = players.bot.node.get_player(
        await fetch_player(int(session["me"].get("id")))
    )
    ...
    if not player:
        return (
            json.dumps(
                {
                    "message": "Player not found. Connect to same voice and try again",
                    "code": 404,
                }
            ),
            404,
...
async def suggestions():
    ...
    return json.dumps(
        [
            players.bot.build.track(track.info, track.track_type.value)
            for track in query
        ][:20]
    )
...
async def queue_render():
    ...
    if player:
        tracks = [
            players.bot.build.track(track.info, track.track_type.value, track.thumbnail)
            for track in player.queue._queue
        ]
    else:
        tracks = []
