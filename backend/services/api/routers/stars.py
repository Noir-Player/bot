...
async def get_stars(
    ...
    stars = (
        router.db.stars.get_stars(int(router.db.sessions.get(sessionid).get("id")))
        or {}
    )

    stars["tracks"] = stars.get("tracks", [])[page * (count - 1) :][:page]

    return {"data": stars, "meta": {"page": page, "count": count}}
...
@router.get(
    "/{index}", description="Трек из звездочек", response_model=StarsTrackResponse
)
async def get_track(index: int, sessionid: str = Cookie()):
