...
apiusers = Router("apiusers", __name__, template_folder="templates", url_prefix="/api/me")
...
async def edit_me():
    ...
    return json.dumps(db.users.edit_user(int(session['me'].get('id')), data.name, data.description, data.theme))
...
async def set_description():
    ...
    return json.dumps(db.users.set_description(data.description, int(session['me'].get('id'))))
