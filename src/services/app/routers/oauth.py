...
async def make_oauth():
    ...
    resp = await make_response(
        await make_response(
            redirect(
                request.args.get(
                    "redirect", session.pop("temp_redirect", "https://noirplayer.su/me")
                )
            )
        )
    )
    ...
    # Set servers
    servers, same, other, mutual = (
        discord.get("https://discordapp.com/api/users/@me/guilds").json(),
        [],
        [],
    ...
    for server in servers:
        if server.get("permissions") & 8 == 8:  # Если есть разрешения администратора
            server["settings"] = db.setup.get_setup(int(session["me"].get("id")))
            try:
    ...
    # Update user profile
    db.users.set_name(session["me"].get("username"), int(session["me"].get("id")))
    ...
    if not devices:
        ...
        db.cache.push(
            {"devices": request.user_agent.string}, int(session["me"].get("id"))
        )
    else:
