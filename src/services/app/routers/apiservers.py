...
apiservers = Router(
    "apiservers", __name__, template_folder="templates", url_prefix="/api/servers"
)
...
async def guild(guild_id):
    ...
    try:
        assert guild_id in [
            int(guild.get("id"))
            for guild in db.cache.get(int(session["me"].get("id")))["guilds"].get(
                "same"
            )
        ]
        ...
        return (
            json.dumps(
                {
                    "message": "Guild not found. Please update guilds info (/update)",
                    "code": 404,
                }
            ),
            404,
...
async def edit_guild(guild_id):
    ...
    try:
        ...
        data["webhook"]["id"] = (
            db.setup.get_setup(guild_id).get("webhook", {}).get("id")
        )
...
async def create_webhook(guild_id):
    ...
    try:
        ...
        return (
            json.dumps(
                {
                    "message": "Guild or channel not found. Please update guilds info (/update)",
                    "code": 404,
                }
            ),
            404,
    ...
    try:
        webhook = apiservers.execute(
            channel.create_webhook(name=data.name, avatar=guild.icon)
        )
