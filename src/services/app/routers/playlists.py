...
async def main():
    ...
    return await render_template("main/playlists.html", playlists=playlists, singles=singles, len=len, shuffle=shuffle)
    ...
    if playlist.get('description'):
        ...
        playlist['html_notes'] = bleach.clean(
            re.sub(
                url_regex,
                '',
                markdown.markdown(playlist.get('description'))
            ),
            tags=tags,
            attributes=attrs
        )

    user_playlists = db.playlists.get_user_playlists(
        int(session['me'].get('id')), False) if session.get('me') else None

    if not session.get('me') or not db.cache.get(int(session['me'].get('id'))):
        add = remove = edit = False
    else:
        edit = int(
            session['me'].get('id')) == int(
            playlist.get('author').get('id'))
        remove = edit or (int(session['me'].get('id')) in playlist.get(
            'forked') if playlist.get('forked') else False)
        add = not remove

    return await render_template("main/playlist.html",
                                 playlist=playlist,
                                 len=len,
                                 addable=add,
                                 removable=remove,
