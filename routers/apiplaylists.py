...
apiplaylists = Router("apiplaylists", __name__, template_folder="templates", url_prefix="/api/playlists")

def authorize(obj: dict):
    obj['author'] = {"id": int(session['me'].get('id')), "name": session['me'].get('username')}
    return obj
...
async def my_playlists():
    ...
    for playlist in db.playlists.get_user_playlists(
...
async def create_playlist():
    ...
    return json.dumps(db.playlists.create_playlist(id=int(session['me'].get('id')),name=session['me'].get('username'), **playlist.model_dump()))
...
async def edit_playlist(uuid):
    try:
        ...
        return json.dumps(db.playlists.edit_playlist(uuid=uuid, id=int(session['me'].get('id')), **playlist.model_dump(exclude=['uuid', 'author', 'tracks', 'tracks', 'metric', 'forked', 'integer'])))
    except:
        traceback.print_exc()
...
async def add_to_playlist(uuid):
    ...
    try:
        ...
        if type(query) == pomice.Playlist:
            ...
            track = {
                "$each": [
                    apiplaylists.bot.build.track(track.info, track.track_type.value) for track in tracks
                    ]
            }

        elif query:
            track = apiplaylists.bot.build.track(query[0].info, query[0].track_type.value)

        result = db.playlists.add_to_playlist(uuid=uuid, id=int(session['me'].get('id')), track=track)
...
async def remove_track(uuid):
    ...
    return json.dumps(db.playlists.remove_from_playlist(uuid, int(session['me'].get('id')), data.url))
...
async def move_track(uuid):
    ...
    return json.dumps(db.playlists.move_track(uuid, data.url, data.pos, int(session['me'].get('id'))))
...
async def merge_playlists():
    ...
    return json.dumps(db.playlists.merge_playlists(data.uuid1, data.uuid2, int(session['me'].get('id'))))
