...
def get_session_keys(cookies = None):
    ...
    return uuid.uuid5(uuid.NAMESPACE_X500, f'{key}{router.salt}').__str__(), key
...
async def login(request: Request):
    ...
    router.db.sessions.set_oauth_state(state, sessid or request.cookies.get("sessionid"))
...
async def callback(request: Request, sessionid: str = Cookie()):
    ...
    router.db.sessions.set_oauth_token(
        discord.fetch_token(
            TOKEN_URL,
            client_secret=OAUTH2_CLIENT_SECRET,
            authorization_response=request.url.__str__().replace("http://", "https://")
        ),
        sessionid
    )
