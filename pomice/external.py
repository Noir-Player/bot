...
class External:
    ...
    def get_recommendations(self, type: Literal['yt', 'sp'], *args, **kwargs) -> List[Dict[str, str]]:
        """
    ...
    def get_relayted_playlists(self, type: Literal['yt', 'sp'], id: str) -> List[Dict[str, str]]:
        """
        ...
        if type == 'yt':
            ...
            result = self.spotify.search(q=f"playlist:{id}", type="playlist")['playlists']['items']
        ...
        if type == 'yt':
            ...
            for i in playlists:
                if i.get("resultType") == "playlist":
                    result.append(self.ytmusic.get_playlist(i.get("playlistId"), limit=500))
