...
class Filters:
    ...
    def has_filter_type(self, *, filter_type: Filter) -> bool:
        ...
        return any(
            f for f in self._filters if isinstance(
                f, type(filter_type)))
...
class Player(VoiceProtocol):
    ...
    def rate(self) -> float:
        ...
        if _filter := next(
                (f for f in self._filters._filters if isinstance(
                    f,
                    Timescale)),
                None):
    ...
    async def _dispatch_event(self, data: dict) -> None:
        ...
        if isinstance(
                event,
                TrackEndEvent) and event.reason not in (
                "REPLACED",
                "replaced"):
            self._current = None
    ...
    async def _swap_node(self, *, new_node: Node) -> None:
        ...
        self._log.debug(
            f"Swapped all players to new node {new_node._identifier}.")
    ...
    async def get_tracks(
        ...
        *,
        ...
        """
        return await self._node.get_tracks(query, ctx=ctx, requester=requester, search_type=search_type, filters=filters)
    ...
    async def add_filter(self, _filter: Filter, fast_apply: bool = False) -> Filters:
        ...
        self._log.debug(
            f"Filter has been applied to player with tag {_filter.tag}")
        if fast_apply:
            self._log.debug(
                f"Fast apply passed, now applying filter instantly.")
    ...
    async def remove_filter(self, filter_tag: str, fast_apply: bool = False) -> Filters:
        ...
        if fast_apply:
            self._log.debug(
                f"Fast apply passed, now removing filter instantly.")
            await self.seek(self.position)
    ...
    async def edit_filter(
        ...
        *,
        ...
        self._filters.edit_filter(
            filter_tag=filter_tag,
            to_apply=edited_filter)
        payload = self._filters.get_all_payloads()
        await self._node.send(
            method="PATCH",
            path=self._player_endpoint_uri,
        ...
        if fast_apply:
            self._log.debug(
                f"Fast apply passed, now editing filter instantly.")
            await self.seek(self.position)
