...
class Node:
    ...
    async def _handle_node_switch(self) -> None:
        nodes = [node for node in self.pool._nodes.copy().values()
    ...
    async def _configure_resuming(self) -> None:
        ...
        if self._version.major == 3:
            ...
            self._log.warning(
                "Using a resume key with Lavalink v4 is deprecated.")
            data["resuming"] = True
    ...
    async def connect(self, *, reconnect: bool = False) -> Node:
        ...
        try:
            ...
            if reconnect:
                self._log.debug(
                    f"Trying to reconnect to Node {self._identifier}...")
    ...
    async def get_tracks(
        ...
        *,
        ...
        if discord_url := URLRegex.DISCORD_MP3_URL.match(query):
            ...
            if not URLRegex.BASE_URL.match(query) and not URLRegex.LAVALINK_SEARCH.match(
                    query) and not URLRegex.LAVALINK_REC.match(query):
    ...
    async def get_recommendations(
        ...
        **kwargs
        ...
        if track.track_type == TrackType.SPOTIFY:
            ...
            return await self.get_tracks(query=query, ctx=ctx or track.ctx, requester=requester)

        elif track.track_type == TrackType.YOUTUBE:
            if not playlist_id:
                query = self._ytm_client.get_watch_playlist(
                    videoId=track.identifier, **kwargs)['tracks']
            else:
                query = self._ytm_client.get_watch_playlist(
                    playlistId=playlist_id, **kwargs)['tracks']
            ...
            for song in query:
                tracks.append(
                                (
                                    await self.get_tracks(f"https://music.youtube.com/watch?v={song['videoId']}", ctx=ctx, requester=requester)
                                )[0]
                            )
...
class NodePool:
    ...
    def get_node(cls, *, identifier: Optional[str] = None) -> Node:
        ...
        """
        available_nodes = {identifier: node for identifier,
                           node in cls._nodes.items() if node._available}
